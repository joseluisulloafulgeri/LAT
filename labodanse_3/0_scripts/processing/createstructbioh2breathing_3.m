...
% createstructurebioh2breathing.m
% version for May 2015 data
...
% so far, to run in /Users/labodance/Documents/DataAnalysis/transfering_files_labodanse_3/1_data_organized

% remember the data is organized as follows:
% /Users/labodance/Documents/DataAnalysis/transfering_files_labodanse_3/1_data_organized/2015_05_12_Yoga/2_bioh_capteurs/A11_573_Breathing.mat
...
for i_session = 1:length(directoryNames) % loop for each session
    ...
    for i_subject = 1:length(directoryNames3) % loop for each subject
        ...
        %% Importing data
        load(directoryNames3{i_subject}) % as BreathingfullData
        % there is always Breathing and ECG data (as separated .mat files)
        
        %% Gathering time info
        % we save the timelines of each subject
        dataTimeSecondsMat{i_subject} = BreathingfullData(:,1);
        ...
        %% Gathering data
        dataOfInterest = {'Breathing'};
        % get save the data in a cell array
        dataFirstStep{i_subject} = BreathingfullData(:, 2);
    ...
    compilatedSeconds = 0:1/25:(length(selectedDataset)/25)-1/25;
    ...
    processingSteps = cellstr(['structure created on ', date, ' with createstructbioh2.m']);   
    
    % name of the data-file
    fileName = sprintf('Breathing_%s', directoryNames{i_session}); % regular version
    % fileName = sprintf('Breathing_%s_alt', directoryNames{i_session}); % for alternative versions
