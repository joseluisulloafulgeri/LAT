...
markersOfInterest = {'duo_1'};
markersOfInterestNames = {'duo 1'};
% data of interest
dataType = {'HR' 'BR'};
%dataType = {'x' 'y'};
...
correlationType = {'1'}; % corrcoef == 1; partialcorr == 2
...
for i_measures = 1:length(correlationsMeasures)
    for i_dataType = 1:length(dataType)
        for i_lag = 1:nFullWindows
            ...
            eachMeasuresDance = zeros(length(sessions)*length(markersOfInterest), 9);
            counter = 1;
            for i_dance = 1:length(markersOfInterest)
                for i_sessions = 1:length(sessions)
                    
                    file = sprintf('dens_%s_ses%d_%s_lag00%d_%sdata.mat', ...
                        correlationsMeasures{i_measures}, i_sessions, markersOfInterest{i_dance}, i_lag, dataType{i_dataType});
                    % open file
                    load(file);
                    selectedData = allDensities(:, 1); % might not be all the data
                    eachMeasuresDance(counter,:) = selectedData';
                    counter = counter + 1;
                    
                end
            end
            %save
            nameData = sprintf('EachD_%s_lag00%d_%sdata', ...
                correlationsMeasures{i_measures}, i_lag, dataType{i_dataType});
            save(nameData, 'eachMeasuresDance');
        end
    end
end

movefile('EachD_*.mat', '../Each_D')
cd ..
