function plotcorrmatrix(data, statsOption, statsPvalue, statsThreshold, statsThresholdName, corrMethod, dataType, danceType, danceTypeName, TOI, subjectList, session, inputFolder, outputFolder)
% plotcorrmatrix.m
...
if statsOption == 0
    if strcmp(corrMethod, 'corrcoef')
        nameFigure = sprintf('MC1_session%d_%s_%s_%sdata.png', session, danceType, TOI, char(dataType));
    elseif strcmp(corrMethod, 'partialcorr')
        nameFigure = sprintf('MC2_session%d_%s_%s_%sdata.png', session, danceType, TOI, char(dataType));
    end
    
elseif statsOption == 1
    if strcmp(corrMethod, 'corrcoef')
        nameFigure = sprintf('MC1_session%d_%s_%s_%sdata_stat1thr%s.png', session, danceType, TOI, char(dataType), statsThresholdName);
    elseif strcmp(corrMethod, 'partialcorr')
        nameFigure = sprintf('MC2_session%d_%s_%s_%sdata_stat1thr%s.png', session, danceType, TOI, char(dataType), statsThresholdName);
    end
    
elseif statsOption == 2
    if strcmp(corrMethod, 'corrcoef')
        nameFigure = sprintf('MC1_session%d_%s_%s_%sdata_stat2thr%s.png', session, danceType, TOI, char(dataType), statsThresholdName);
    elseif strcmp(corrMethod, 'partialcorr')
        nameFigure = sprintf('MC2_session%d_%s_%s_%sdata_stat2thr%s.png', session, danceType, TOI, char(dataType), statsThresholdName);
    end
end
