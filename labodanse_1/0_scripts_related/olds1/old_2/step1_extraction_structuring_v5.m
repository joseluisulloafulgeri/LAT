...
%        timeline_true = ones(1, length(extended_timeline));
        ...
        for i_step = 1:length(extended_timeline)
            if test_member(i_step) == 1;
                value_eng(i_step) = imported_data.data(index_in_rounded(i_step), 2); % 1 x 1 901 199
            else
                % nothing
            end
        end
        ...
        count = 1;
        value_eng_2 = NaN(1, floor(length(extended_timeline)/100)); % 1 901
        for i_new_points = 1:100:floor(length(extended_timeline)/100)*100 % 1 901 199
            value_eng_2(count) = max(value_eng(i_new_points:i_new_points + 99));
            count = count + 1;
        end
...
%         data_first_step{i_subject} = data_all(:, index_selection);
    ...
    lenght_data = min(common_timepoints(oi_subjects)) - 2; % e.g. 11575 datapoints -- ask Yann why -1? -2 to be sure 
...
%        selected_dataset(:,:,n_subject) = data_first_step{n_subject}(min_time_index(n_subject):min_time_index(n_subject) + lenght_data, :);
    ...
    start_epoch = selected_dataset(1,1); % get the first epoch data that is common across all subjects,
    ...
    for j_new_date = 1:length(selected_dataset)
        epoch_hours = floor(selected_dataset(j_new_date,1)/60*60); % get the hours,
        epoch_minutes = floor(selected_dataset(j_new_date,1) - epoch_hours *60*60); % get the minutes,
        epoch_seconds = floor(selected_dataset(j_new_date,1) - epoch_hours*60*60 - epoch_minutes*60); % get the seconds,
        news_epochs{j_new_date} = [num2str(start_epoch_hours),'-',num2str(start_epoch_minutes),'-',num2str(start_epoch_seconds)];
     end
    ...
    structured_data = struct( ...
        ...
        'common_timepoints', common_timepoints(oi_subjects), ...
