...
for x_sessions = 1:length(folders_level_1) % HERE start the loop for each session
    ...
    for i_subject = 1:length(folders_level_3) % HERE start the loop for each subject
        ...
        %% 5. gathering time info of each subject
        
        %% 5A. rounding the subject timeseries
        
        % time correction => x + 7200000
        correction = 7200000;
        corrected_timeline = bsxfun(@plus, imported_data.data(:,1), correction);
        
        extract_timeseries = {};
        for j_count = 1:length(corrected_timeline)
            extract_timeseries{j_count} = sprintf('%.0f', corrected_timeline(j_count,1));
            rounded_timeseries(j_count) = round( str2num(extract_timeseries{j_count})/100 ) *100;
        end
                
        %extract_timeseries = extract_timeseries';
        %rounded_timeseries = rounded_timeseries';

        %% 5B. generating the long timeseries
        
        % get first timepoint
        first_timepoint = imported_data.data(1,1);
        last_timepoint = imported_data.data(end,1);
        % time correction => x + 7200000
        first_timepoint_corr =  first_timepoint + 7200000;
        last_timepoint_corr = last_timepoint + 7200000;
        % round this first timepoint to have e.g. a 100 ms point, instead of a 102 ms point
        new_first_timepoint_A = mat2str(first_timepoint_corr);
        new_first_timepoint_B = round( str2num(new_first_timepoint_A)/100 ) *100; % 1413824999102 -> 1413824999100
        %
        new_last_timepoint_A = mat2str(last_timepoint_corr);
        new_last_timetpoint_B = round( str2num(new_last_timepoint_A)/100 ) *100;
        time_length = new_last_timetpoint_B - new_first_timepoint_B;
        
        pre_new_timeline = 0:100:time_length-1;
        % we generate a rounded version of the original timeline
        whole_timeline = bsxfun(@plus, pre_new_timeline, new_first_timepoint_B);
        
        %% e
        
        % remember the data is in imported_data.data(:,2) for X & imported_data.data(:,3) for Y
        
        value_eng = ones(1, length(whole_timeline));
        
        % test
        [test_member, index_in_rounded] = ismember(whole_timeline, rounded_timeseries);
        
        for i_step = 1:length(whole_timeline)
        ...
        value_eng_2 = value_eng;
        
        for i_recover = 1:length(value_eng)
            if value_eng(i_recover) == 1;
                if value_eng(i_recover-1) > 1;
                    if value_eng(i_recover+1) > 1;
                        value_eng_2(i_recover) = value_eng(i_recover-1);
                    else %
                        value_eng_2(i_recover) = 1;
                    end
                end
                
             elseif value_eng(i_recover) > 1;
                value_eng_2(i_recover) = value_eng(i_recover);
                
            end
        end
        
        value_eng_3 = medfilt1(value_eng_2,90,30);       
        
        
                
        % first, we get the column with the timestamps
        unconverted_time = imported_data.data(:,1);
        
        % second, we eliminate the last three digits
        unconverted_time_2 = fix(unconverted_time/1.e3);
        
        % third, we convert them to human-readable format
        converted_time = datestr(unconverted_time_2/86400 + datenum(1970,1,1));
        ...
        recovering_ms = num2str(unconverted_time);
        recovering_ms_2 = recovering_ms(:, 10:13);
        recovering_ms_3 = str2num(recovering_ms_2);
        recovering_ms_4 = recovering_ms_3/1000;        
        
        %% 6. re-taking the old way,
        ...
        for xo_date = 1:length(time_data)
            ...
            data_time_seconds{xo_date} = str2num(segmented_date{xo_date}{1})*60*60 + str2num(segmented_date{xo_date}{2})*60 + recovering_ms_4(xo_date);
        end
