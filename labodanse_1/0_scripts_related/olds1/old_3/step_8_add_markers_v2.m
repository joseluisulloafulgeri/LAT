...
for i_struct = 1:length(files_names_level_1)
    ...
    for i_mark = 1:length(time_markers_info) % e.g. 8
        ...
        if time_markers_info(i_mark) < last_index %%% FOR-SHORTER-SEQUENCES
            time_markers_index(i_mark) = find(fileStruct.common_seconds >= time_markers_info(i_mark),1,'first'); %%% YANN-DEFAULT
            
            if i_mark ~= length(time_markers_info) % e.g. ~= 8 %%% YANN-DEFAULT
                if time_markers_info(i_mark +1) < last_index %%% FOR-SHORTER-SEQUENCES
                    time_markers_index(i_mark +1) = find(fileStruct.common_seconds >= time_markers_info(i_mark +1), 1, 'first'); %%% YANN-DEFAULT
                end
            else
            ...
            if i_mark ~= length(time_markers_info) % i_mark < 8 %%% YANN-DEFAULT
    ...
    if isfield(fileStruct,'marker_tab') == 0
        fileStruct.marker_tab{1} = time_markers_cell;
        fileStruct.marker_type{1} = time_markers_info;
    else
        fileStruct.marker_tab{size(fileStruct.marker_tab,1)+1} = time_markers_cell;
