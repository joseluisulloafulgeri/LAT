...
function IBItimeseries= createECGmat_5(filesPathName, dataOfInterest)
...
elseif dataOfInterest == 3,
    ...
    IBIintervals_bioH = [initialLag; str2double(fileTabRaw{1})]; % convert string to double
end
...
% matrix
timeSeriesSOI = [timeAxisECG, fileTabECG{7+dataOfInterest-2}];
% Since the script only deals with one single data of interest,
...
figure, 
subplot(211)
plot(IBIintervals_bioH/1000,'m','DisplayName','RR IBI')
ylim([0 2])
ylabel('time [s]')
xlabel('time [s]')
...
subplot(212)
plot(locsSeconds(2:end),IBIintervals)
...
% % % %% 
% % % %% interpolation of IBI mat 
% % % 
% % % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % % CREATE BB TIME SERIES - INTERPOLATION OF BB DATA
% % % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % 
% % % % Interpolate the intervalBeat to a new, fixed sample rate : fsIntervalBeat
% % % 
% % % fsIntervalBeat = 1; % desired sampling rate
% % % 
% % % % Create new syncronized timeLine for BB time series and BBsecondsIntervals.
% % % % Each timeLine is incremented of both timeStartSummary (given by the
% % % % summary file) and shiftedTime (given by the correlation between summary
% % % % Br data and BPM time series obtained via the BB.csv file)
% % % % Then, the "- lengthWindow/2/fsBB" comes from the fact that HR/BR window
% % % % time index is taken at the middle of the window => since we are now
% % % % creating non meaned time series, their timeLine starts at the beginning of the
% % % % window...
% % % 
% % % % % newIBItimeline = (0 :1/fsIntervalBeat: (floor(length(IBItime)/1000)-1)/fsIntervalBeat) + timeStartSummary + shiftedTime - lengthWindow/2/1000; % gives a resampled and synced timeLine for BBintervals.
% % % newIBItimeline = timeStartECG : 1/fsIntervalBeat : timeStartECG + locsSeconds(end); % ! ! ! !  Comment expand ??? Prendre la fin ? Prendre la durée de tous les IBI cumulés ? ??
% % % % % % % rectifiedIBIsecondIntervals = IBIsecondsIntervals ;% here was a big problem.. this was added here : + timeStartSummary + shiftedTime - lengthWindow/2/1000; % BB intervals rectified relative to "summary" start % !! 2/1000 ou *1000 ?? Initialement : * 1000
% % % % % % % IBIintervalsTime = IBItime + 1000*(timeStartSummary + shiftedTime - lengthWindow/2/1000);
% % % 
% % % % interpolation of RR/BB data
% % % % yi = interp1(x,Y,xi,method)
% % % % interpolates to find yi, the values of the function Y at the points in the vector or array xi
% % % % a timeseries [yi] (e.g. 1x 4495) is created with the newBBtimeline [xi] (4495) as
% % % % timeline, taking the original BB values [Y] (BBintervals), putting them at the corresponding
% % % % timepoints (x), and interpolating the rest of the data,
% % % 
% % % % 'pchip' and 'linear' interp1 methods both works well,
% % % 
% % % % newIBItimeseries = interp1(rectifiedIBIsecondIntervals, IBIintervals(:,1)', newIBItimeline, 'linear');
% % % newIBItimeseries = interp1(IBIintervalsTime, IBIintervals(:,1)', newIBItimeline, 'linear');
% % % 
% % % % putting all together
% % % IBItimeseries = [newIBItimeline ; newIBItimeseries]';
% % % size(IBItimeseries)
% % % IBIintervals = [IBIintervalsTime', IBIline];
% % % size(IBIintervals)
% % % 
% % % disp('l.217')
% % % pause
...
% % % % 
% % % % % initialisation
% % % % IBIintervalsRefilled = IBIintervals'; % duplicate the IBIintervals array so as to keep the IBIintervals array untouched
% % % % IBIintervals = IBIintervals';
% % % % 
...
% % % % % % 
% % % % % % % create the time line
% % % % % % IBIline = zeros(length(ECGsignal,1); % IBIline contains as many index as milliseconds
% % % % % % 
% % % % % % % generate timeline with the duration of IBI file, and taking the first timepoint of the Summary file
% % % % % % % IBItime = ECGtime ! 
% % % % % % % Create the IBI timeline (IBIline): distribute the IBI intervals in the created timeline
...
% % % % % %     IBIline(compiledMiliSec) = IBIintervals(ii); % set IBIline to the IBIinteravls value, for each heart/breathing event
% % % % % %     
% % % % % %     IBIsecondsIntervals(ii) = compiledSec; % store the time of each heart/breathing event [s]
% % % % % % end
% % % 
% % % 
% % % 
...
% % % 
% % % sortedIBIindex = sortrows(IBIintervals'); % sort IBI values in an ascending mode
