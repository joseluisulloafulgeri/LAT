...
%selected_folders = {'2014_10_21_PM1'};
...
for x_sessions = 1:length(folders_level_1) % HERE start the loop for each session
    ...
    min_time_index = []; max_time_index = []; common_timepoints = []; structure_comments = {};
    selected_dataset = [];
    ...
    for i_subject = 1:length(folders_level_3) % HERE start the loop for each subject
        ...
        % TEST-to erase... imported_data = importdata('B11_476_PM1_ENG.csv');
        ...
        %value_eng_1 = NaN(1, length(extended_timeline));
        value_eng = NaN(2, length(extended_timeline));
%        timeline_true = ones(1, length(extended_timeline));
        ...
        for i_step = 1:length(extended_timeline)
            if test_member(i_step) == 1;
                value_eng(1, i_step) = imported_data.data(index_in_rounded(i_step), 2); % 1 x 1 901 199
                value_eng(2, i_step) = imported_data.data(index_in_rounded(i_step), 3); % 1 x 1 901 199
            else
                % nothing
            end
        end
        
        % taking the maximal value each 100 points == 100 ms
        count = 1;
        value_eng_2 = NaN(2, floor(length(extended_timeline)/100)); % 1 901
        for i_new_points = 1:100:floor(length(extended_timeline)/100)*100 % 1 901 199
            value_eng_2(1, count) = max(value_eng(1, i_new_points:i_new_points + 99));
            value_eng_2(2, count) = max(value_eng(2, i_new_points:i_new_points + 99));
            count = count + 1;
        end
        ...
        % TIMELINE NOW
        pre_new_timeline = 0:100:floor(length(extended_timeline)/100)*100-1;

        % round first time point
        new_first_timepoint_A = mat2str(corrected_data_timeline(1));
        new_first_timepoint_B = round( str2num(new_first_timepoint_A)/100 ) *100; % 1413824999102 -> 1413824999100
        
        % regenerate a good timeline
        new_timeline = bsxfun(@plus, pre_new_timeline, new_first_timepoint_B);
...
%       converted_time = datestr(unconverted_time/86400 + datenum(1970,1,1));
        converted_time = datestr(unconverted_time/86400000 + datenum(1970,1,1));
        ...
        % recovering the miliseconds
                
        recovering_ms = {}; recovering_ms_2 = {}; recovering_ms_3 = [];
        for i_conv = 1:length(unconverted_time)
            recovering_ms{i_conv} = num2str(unconverted_time(i_conv));
            recovering_ms_2{i_conv} = recovering_ms{i_conv}(:, 11:13);
            recovering_ms_3(i_conv) = str2num(recovering_ms_2{i_conv});
        end
...
%            data_time_seconds{xo_date} = str2num(segmented_date{xo_date}{1})*60*60 + str2num(segmented_date{xo_date}{2})*60 + str2num(segmented_date{xo_date}{3});
            data_time_seconds{xo_date} = str2num(segmented_date{xo_date}{1})*60*60 + str2num(segmented_date{xo_date}{2})*60  + str2num(segmented_date{xo_date}{3}) + recovering_ms_3(xo_date)/1000;
        end
...
%         data_first_step{i_subject} = data_all(:, index_selection);
    ...
    minutes_common_A = (common_max_time - common_min_time)/60; % get the minutes,
    minutes_common_B = floor(minutes_common_A); % get the minutes,
    seconds_common = (minutes_common_A -minutes_common_B)*60; % get the seconds,
    
    structure_comments{3} = horzcat('length of the common session: ', num2str(minutes_common_B), 'min ', num2str(seconds_common), 'sec');
    
    %% 12. selection of data with common datapoints
    
    lenght_data = min(common_timepoints(oi_subjects)) - 1; % e.g. 11575 datapoints -- ask Yann why -1? -2 to be sure 
...
%        selected_dataset(:, n_subject) = data_first_step{n_subject}(min_time_index(n_subject):min_time_index(n_subject) + lenght_data);
    ...
    new_compiled_seconds_start = data_time_seconds_mat{1}(min_time_index(1));
    % correction relative the time markers, added 01/12/2014
    new_compiled_seconds_start = new_compiled_seconds_start - 60;
    
    new_epochs = {}; counter = 0;
    new_compilated_seconds_char = {};

    for j_new_date = 1:length(selected_dataset)
        time_epoch = new_compiled_seconds_start + counter;
 %       new_compilated_seconds_char{j_new_date} = num2str(time_epoch);
        ...
        new_epochs{j_new_date} = [num2str(epoch_hours_B),':',num2str(epoch_minutes_B),':',num2str(epoch_seconds_A)];
        counter = counter + 0.1;
    end

    compilated_seconds_A = [];
    compilated_seconds_A = 0:0.1:(length(selected_dataset)/10)-0.1;
    compilated_seconds_B = bsxfun(@plus, compilated_seconds_A, new_compiled_seconds_start);
    
    common_compilated_seconds = compilated_seconds_B';
    common_timeline = new_epochs';
    ...
    structure_date = folders_level_1{x_sessions};
    ...
    fileStruct = struct( ...
        ...
        'common_seconds', common_compilated_seconds, ...
