...
for ii = 3 : length(InFold0)
    ...
    %    
        ...
        for kk = 3 : length(InFold2)
            ...
            TreeName4 = [TreeName3,'/',InFold3(3).name] % [3] car le seul dossier présent est le bon. Ce n'est pas forcément le cas !!!
           ...
           [yr,mt,da,hr,mn,sc,daTyp] = strread(InFold4(20).name,'%f%f%f-%f%f%f%s','delimiter','_');
            ...
            dataFileTab = cell2mat(fileTab(sign2use)); % sélection des colonnes de valeurs à structurer
        ...
        for nn = 1 : nSubject
            ...
            commonTimeIndex (nn) =  commonMaxTimeIndex(nn)-commonMinTimeIndex(nn); % verifier le timestamp(commonTimeIndex) de chaque sujet, et les comparer => ils doivent être quasi identiques.
        end
    ...
    %% déterminer l'heure de commencement à partir du TimeAxis du fichier
