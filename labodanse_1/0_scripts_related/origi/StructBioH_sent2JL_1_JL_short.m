...
for i_session = 3 : length(InFold0)
    
    TreeName1 = [TreeName0,'/',InFold0(i_session).name] % concatenate the second level directory, with the files / folders found in the first level dir.
    InFold1 = dir (TreeName1);
    
    [yr2,mt2,da2,sss]=strread(InFold0(i_session).name,'%f%f%f%s','delimiter','_'); % read folder infos to get the date
    ...
    %    
    %    
        TreeName2 = [TreeName1,'/',InFold1(j_datatype).name]
        InFold2 = dir (TreeName2);
        
        [num,dataTyp,extraInfo] = strread(InFold1(j_datatype).name,'%f%s%s','delimiter','_'); % extract data type name to complete both futur structure name and dataType field
        ...
        for k_sujet = 3 : length(InFold2)

            subj4struct{k_sujet-2} = InFold2(k_sujet).name; % list each subject number in a cell for future structure field listSubj
            
            TreeName3 = [TreeName2,'/',InFold2(k_sujet).name]
            ...
            %% files : 
           [yr,mt,da,hr,mn,sc,daTyp] = strread(InFold4(20).name,'%f%f%f-%f%f%f%s','delimiter','_');
       	
            dirNam = [TreeName4,'/',InFold4(20).name]
            dirNameTab4struct{k_sujet-2}= dirNam; % list each file directory in a cell for future structure field source
            ...
            data2use{1, k_sujet-2}=num2cell([timeAxis, dataFileTab]) ; %[1 2 14 15 31]+6;

            minTimeTab (k_sujet-2) = timeAxis(1); % get the starting time of the current file
            maxTimeTab (k_sujet-2) = timeAxis(length(timeAxis)); % get the end time of the file
