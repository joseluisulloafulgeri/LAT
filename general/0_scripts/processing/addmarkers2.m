...
% addmarkers2.m adapted from addmarkers.m
...
% so far, to run in 
% /Users/labodance/Documents/DataAnalysis/transfering_files_labodanse_3/2_data_analysis/bioharness/bioharness_1
% /Users/labodance/Documents/DataAnalysis/transfering_files_labodanse/2_data_analysis/bioharness_new/bioharness_1
% /Users/labodance/Documents/DataAnalysis/transfering_files_labodanse_2/2_data_analysis/bioharness_new/bioharness_1

% created: modified: JLUF 08/06/2015
% updates:
...
%files = dir('*Breathing*'); % e.g. '*20_PM*' or '*21_PM1*' '*HRBR*21_PM2*'
files = dir('*.mat');
...
%for i_struct = 8:8   
    ...
    if regexp(structName, 'alt') > 0
        structName = structName(1:end-4);
    end
    
    if regexp(structName, 'Breathing') > 0
        markersFileName = sprintf('%s_%s.csv', markerType, char(structName(11:end)));
    elseif regexp(structName, 'ECG') > 0
        markersFileName = sprintf('%s_%s.csv', markerType, char(structName(5:end)));
    elseif regexp(FileStruct.name, 'HRBR') > 0
        markersFileName = sprintf('%s_%s.csv', markerType, char(structName(6:end)));
    end
